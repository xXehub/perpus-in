...
def tambah_buku():  # menambahkan data ke csv
 ...
 # --------------- ini untuk fungsi auto memberi number ------------------------
        ...
        with open(perpus_csv, mode="r", newline='') as csv_file:
            ...
            for row in csv_reader:
                buku_sakkarepmu.append(row)
        row_count = sum(1 for row in buku_sakkarepmu) # membaca total row yang ada di csvs
...
#   \___\__,_|_| |_| |___/\_,_|_\_\\_,_|''')
    judul_buku = input('''
   ___          _   ___ _                            
  / __|__ _ _ _(_) / __(_)_ __  ___ _ _ _ __ _  _ ___
 | (__/ _` | '_| | \__ \ | '_ \/ -_) '_| '_ \ || (_-<
  \___\__,_|_| |_| |___/_| .__/\___|_| | .__/\_,_/__/
                         |_|           |_|                                                 
   [?] cari buku berdasrakan judul : ''').title()
    ketemu = []
    ...
    if len(ketemu) > 0:
        ...
        # with open(perpus_csv, "r") as fp: 
        #     x = from_csv(fp)
        # print (x.get_string())(fields=["Judul Buku" ])
        # # print(x)
    #     print(''' 
    # ╔════════════════╗
    # ║ data ditemukan ║ 
    # ╚════════════════╝ ''')
        print(''' 
  \t  ___       _          ___  _ _                 _             
 \t |   \ __ _| |_ __ _  |   \(_) |_ ___ _ __ _  _| |____ _ _ _  
 \t | |) / _` |  _/ _` | | |) | |  _/ -_) '  \ || | / / _` | ' \ 
 \t |___/\__,_|\__\__,_| |___/|_|\__\___|_|_|_\_,_|_\_\__,_|_||_| ''')
        print('''+------------------------------------------------------------------------------------+''')
        print(f'''[-] nomor buku      : {ketemu['No']}''', end='')
        print(f'''\t\t\t [-] kode buku       : {ketemu['Kode Buku']}''')
        print(f'''[-] tanggal masuk   : {ketemu['Tanggal Masuk']}''', end='')
        print(f'''\t [-] judul buku      : {ketemu['Judul Buku']}''')
        print(f'''[-] pengarang       : {ketemu['Pengarang']}''', end='')
        print(f'''\t [-] penerbit        : {ketemu['Penerbit']}''')
        print(f'''[-] jumlah halaman  : {ketemu['Jumlah Halaman']}''', end='')
        print(f'''\t [-] tahun terbit    : {ketemu['Tahun Terbit']}''')
        takon.cari_buku_ulang_1()
    else:
        print(''' 
    ╔═════════════╗ input kamu tidak diketahui
    ║ input salah ║ coba lagi, jalankan ulang
    ╚═════════════╝ program dan cek kembali ^ ^! ''')
        takon.cari_buku_ulang_2()
    takon.mbalek_menu()

def edit_buku():  # definisi fungsi edit data / buku
    clear_screen()
    print(''' 
 \t\t\t  ___    _ _ _     ___        __ _              ___      _        
 \t\t\t | __|__| (_) |_  |   \ __ _ / _| |_ __ _ _ _  | _ )_  _| |___  _ 
 \t\t\t | _|/ _` | |  _| | |) / _` |  _|  _/ _` | '_| | _ \ || | / / || |
 \t\t\t |___\__,_|_|\__| |___/\__,_|_|  \__\__,_|_|   |___/\_,_|_\_\\_,_|''')
    buku_sakkarepmu = []
    with open(perpus_csv, mode="r", newline='') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        for row in csv_reader:
    ...
    #         f"{data['No']}. \t {data['Kode Buku']} \t\t {data['Tanggal Masuk']} \t\t  {data['Judul Buku']} \t\t {data['Pengarang']} \t {data['Penerbit']} \t\t\t {data['Jumlah Halaman']} Halaman \t\t  {data['Tahun Terbit']} \t")
        ...
        nomer = row_count
        print('''[#] buku nomor            : ''', nomer)  # memanggil nomor buku dari tabel
        print('''+------------------------------------------+''')
        tanggal_masuk = input("[-] tanggal masuk baru    : ")
        judul_buku = input("[-] judul buku baru       : ")
        pengarang = input("[-] pengarang baru        : ")
        penerbit = input("[-] penerbit baru         : ")
        jumlah_halaman = input("[-] jumlah halaman baru   : ")
        tahun_terbit = input("[-] tahun terbit baru      : ")
...
def delete_buku():
    ...
    print(''' 
 \t\t\t\t     _  _                     ___      _        
 \t\t\t\t    | || |__ _ _ __ _  _ ___ | _ )_  _| |___  _ 
 \t\t\t\t    | __ / _` | '_ \ || (_-< | _ \ || | / / || |
 \t\t\t\t    |_||_\__,_| .__/\_,_/__/ |___/\_,_|_\_\\_,_|
  \t\t\t\t             |_|                               ''')
    buku_sakkarepmu = []
    with open(perpus_csv, mode="r") as csv_file:
        csv_reader = csv.DictReader(csv_file)
